...
void DFA::init_state_token_map() {
    ...
    state_token_map[78] = COMMA;
}

void DFA::init_state_machine() {
...
TokenType DFA::get_token_type(int state) {
...
void DFA::print_state_machine() {
    for (int s = 0; s < STATES; s++) {
        cout << "State " << s << ": ";
        for (int c = 0; c < ASCII; c++) {
            if (state_machine[s][c] != -1) {
                cout << " '" << static_cast<char>(c) << "' -> " << state_machine[s][c];
            }
        }
        cout << endl;
    }
...
//class Lexer
Lexer::Lexer(string fName): inFile(fName) {
    if (!inFile.is_open()) {
